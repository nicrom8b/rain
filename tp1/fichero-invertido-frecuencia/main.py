...
def create_inverted_index(pdf_files: List[Path]) -> Dict[str, Dict[str, int]]:
    """
    Crea un índice invertido con frecuencias de palabras por documento.
    ...
    Returns:
        ...
        donde las claves son nombres de PDF y los valores son frecuencias.
    """
    ...
    for pdf_file in pdf_files:
        try:
            ...
            # Contar frecuencias para este documento
            for token in tokens:
                if token not in inverted_index:
                    inverted_index[token] = {}
                if pdf_file.name not in inverted_index[token]:
                    inverted_index[token][pdf_file.name] = 0
                inverted_index[token][pdf_file.name] += 1
...
def save_inverted_index(inverted_index: Dict[str, Dict[str, int]], output_file: Path) -> None:
    """
